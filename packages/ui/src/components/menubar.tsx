...
const Menubar = ({ className, ...props }: React.ComponentProps<typeof MenubarPrimitive.Root>) => (
    <MenubarPrimitive.Root
      className={cn(
        'bg-background border flex gap-1 h-9 items-center p-1 rounded-md shadow-xs',
        className
      )}
      data-slot='menubar'
      {...props}
  ...
  MenubarContent = ({
    ...
    <MenubarPortal>
      <MenubarPrimitive.Content
        ...
        className={cn(
          'bg-popover border data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 min-w-[12rem] overflow-hidden p-1 rounded-md shadow-md text-popover-foreground z-50',
          className
        )}
